...
function model_mcm_forumlation!(model::Model, C::Vector{Int},
    ...
    # C6a - C6b
    if max_dsp == 0
        @constraint(model, [a in (!one_is_output):NA, j in 1:NO], ca[a] <= C[j] + (1-oaj[a,j])*maximum_value)
        @constraint(model, [a in (!one_is_output):NA, j in 1:NO], ca[a] >= C[j] - (1-oaj[a,j])*maximum_target)
        @constraint(model, [j in 1:NO], sum(oaj[a,j] for a in (!one_is_output):NA) == 1)
    else
        @variable(model, use_dsp[1:NO], Bin)
        @constraint(model, [a in (!one_is_output):NA, j in 1:NO], ca[a] <= C[j] + (1-oaj[a,j])*maximum_value + (1-use_dspj[j])*maximum_value)
        @constraint(model, [a in (!one_is_output):NA, j in 1:NO], ca[a] >= C[j] - (1-oaj[a,j])*maximum_target - (1-use_dspj[j])*maximum_target)
        @constraint(model, [j in 1:NO], sum(oaj[a,j] for a in (!one_is_output):NA)+use_dspj[j] == 1)
    ...
    @constraint(model, [n in 1:NA], sum(oaj[a,j] for a in (!one_is_output):n, j in 1:NO) >= NO - (NA-n))
    ...
    if with_pipelining_cost
        ...
        @constraint(model, [a in (!one_is_output):NA], NA*used_as_output[a] >= sum(oaj[a,j] for j in 1:NO))
